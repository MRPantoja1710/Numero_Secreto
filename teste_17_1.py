...
'''
coordenadasx_robos = [10,-20,19,17,-10,17,38,-9]#para manuzear apenas as coordenadas do eixo X
coordenadasy_robos = [2,20,-19,17,18,-32,18,0]#para manuzear apenas as coordenadas do eixo X
coordenadas_robos = [(10,2),(-20,20),(19,-19),(17,17),(-10,18),(17,-32),(38,18),(-9,0)]#Todas as coordenadas
'''

'''
# armazena as coordenadas dos robôs em uma lista/vetor
coordenadasx_robos = [10,-20,19,17,-35,21,-19,29,25,12]#para manuzear apenas as coordenadas do eixo X
coordenadasy_robos = [2,20,-19,17,-20,7,32,1,-10,12]#para manuzear apenas as coordenadas do eixo X
coordenadas_robos = [(10,2),(-20,20),(19,-19),(17,17),(-35,-20),(21,7),(-19,32),(29,1),(25,-10),(12,12)]#Todas as coordenadas
'''
...
def dividir_distancia(x1, y1, x2, y2, n_robos_por_lado, divisoes, va_x, va_y):
            ...
            if(resto_div_inteira == 1):
                for k in range(1, 3):
                    if (k == 1):
                        ...
                        n_robos_por_lado.append(div_inteira)
...
divisoes = dividir_distancia(x1, y1, x2, y2, n)
print("Pontos intermediários:")
for ponto in divisoes:
    print(ponto)
'''
...
#estou dividindo os lados em lado a, b e c
    ...
    n_robos_por_lado = (definindo_robos_por_lado(forma, resto_div_inteira, div_inteira) + 1)#pois se eu colocar n=3, ele vai dividir em 3 partes mas apenas 2 pontos intermediários
    ...
    while(i < num_robos):
        ...
        if(i>=4):#para o resto
            ...
            if (l_andando == 1):
                x1 = coordenadasx_robos[0]
                y1 = coordenadasy_robos[0]
                x2 = coordenadasx_robos[1]
                y2 = coordenadasy_robos[1]

            if (l_andando == 2):
                x1 = coordenadasx_robos[1]
                y1 = coordenadasy_robos[1]
                x2 = coordenadasx_robos[2]
                y2 = coordenadasy_robos[2]


            if (l_andando == 3):
                x1 = coordenadasx_robos[2]
                y1 = coordenadasy_robos[2]
                x2 = coordenadasx_robos[3]
                y2 = coordenadasy_robos[3]


            dividir_distancia(x1, y1, x2, y2, n_robos_por_lado, divisoes, va_x, va_y)
            l_andando += 1
            contador_mudar_de_lado += 1
...
#estou dividindo os lados em lado a, b e c
    div_inteira = int((num_robos-1)/4)#este -1 se refere ao robô que ficará no centro que não irá contar para a formação externa
    resto_div_inteira = ((num_robos-1)%4)
    robo_prox = 0 #Inicializo com 0 mas o primeiro robô irá subistituir o 0
    dist_robo_prox = 10000#Deixo alta pois logo que for comparar com o primeiro robô, ele já será o mais próximo
    i = 0
    while(i < num_robos):
        Ang_Uni_Rob = (360/(4))#angulo que cada robô irá ficar em comparação a outro(Menos 1 pois o primeiro ficará no centro)
        ...
        if(i>0 and i<5):#para os 3 cantos
           radianos =  math.radians((i*Ang_Uni_Rob)+45)#convertendo para radianos
...
#estou dividindo os lados em lado a, b e c
    div_inteira = int((num_robos-1)/5)#este -1 se refere ao robô que ficará no centro que não irá contar para a formação externa
    resto_div_inteira = ((num_robos-1)%5)
    robo_prox = 0 #Inicializo com 0 mas o primeiro robô irá subistituir o 0
    dist_robo_prox = 10000#Deixo alta pois logo que for comparar com o primeiro robô, ele já será o mais próximo
    i = 0
    while(i < num_robos):
        Ang_Uni_Rob = (360/(5))#angulo que cada robô irá ficar em comparação a outro(Menos 1 pois o primeiro ficará no centro)
        ...
        if(i>0 and i<6):#para os 3 cantos
           radianos =  math.radians((i*Ang_Uni_Rob))#convertendo para radianos
...
#estou dividindo os lados em lado a, b e c
    ...
    i = 0
    while(i < num_robos):
        Ang_Uni_Rob = (360/(num_robos-1))#angulo que cada robô irá ficar em comparação a outro(Menos 1 pois o primeiro ficará no centro)
